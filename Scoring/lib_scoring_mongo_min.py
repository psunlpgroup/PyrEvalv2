...
from Preprocess.weiwei_mongo_min import vectorize
from Preprocess.sif_embedding_mongo_min import vectorize_sif
import numpy as np 
import pandas as pd
import glob
...
def vecotirizationProtocol(fname):
    cwd = os.getcwd()
    os.chdir('../Preprocess/')
    #AdT: Commented this line as I am passing the absolute filepath
    # fname = '../Scoring/' + fname
    ...
    os.chdir(cwd)
    return vecs
def vectorizeSCUs(scus, dynamic_base_dir, error_operations_obj):
    reconstruction = {}
    scs = {}
    document = []
    scu_ids = []
    ...
    if not os.path.exists(os.path.join(dynamic_base_dir, 'temp')):
            os.makedirs(os.path.join(dynamic_base_dir, 'temp'))
    tmp_segs_path = dynamic_base_dir + '/temp/tmp.segs'

    with open(tmp_segs_path, 'w') as f:
        for line in document:
            f.write(line + '\n')
    f.close()
    error_operations_obj.temp_segs_creation_stage = 'Temp.segs file creation complete'
    vecs = vecotirizationProtocol(tmp_segs_path)
    j = 0
    for scu_id, length in scu_ids:
        reconstruction[scu_id] = vecs[j:length]
        vecs = vecs[length:]
    return reconstruction, scs
def readPyramid(fname, scoring_dir, config_obj, error_operations_obj):
    global config
    config = config_obj
    soup = getSoup(fname)
    scus = getSCUs(soup)
    recon, scus = vectorizeSCUs(scus, scoring_dir, error_operations_obj)
    #print "recon, ", recon
    scu_objects = []
    layer_sizes = []
...
def score(scus, fn, raw_scores, quality_scores, coverage_scores, comprehension_scores, pyramid, pyramid_name, scu_labels, numsmodel, print_all, log, rf, cu_matches, group_matches, mongodb_operations, student_metadata_obj):
    ...
    if (print_all) or log:
        ...
        # if not print_all:
        #     print("Success!!")
        printEsumLogWrapper(summary_name, segcount, score, quality, coverage, comprehension, q_max, c_max, avg, results, segment_list, num_sentences, segs, scu_labels, pyramid_name, log_f, independentSetValues, print_all, mongodb_operations, student_metadata_obj)
