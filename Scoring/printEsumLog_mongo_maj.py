...
import Scoring.lib_scoring_mongo_min
import os
import textwrap
...
def printScores(scores, output, print_screen, result_obj):
    ...
    result_obj.avg_scu = scores.avg
    result_obj.max_score_avg_scu = scores.cmax
...
def createSentenceMatchDict(segList, pyramid):
    ...
    for s in segList:
        segments_dict = {}
        segments_dict['Sentence ID'] = s.sentence_id
        segments_dict['Segmentation'] = s.segment_id

        for seg_index, text in s.text.items():
            segment_dict = {}
            segment_dict['Segment ID'] = seg_index
            segment_dict['Segment'] = s.text[seg_index]
            if seg_index in s.scu_text_pairs.keys():
                segment_dict['Content Unit'] = s.scu_text_pairs[seg_index]
...
def printEsumLogWrapper(summary_name, segment_count, score, quality, coverage, comprehension, q_max, c_max, avg,
    ...
    v = getInfo(values)
    printEsumLog(s, r, p, v, results, log_file, print_screen, mongodb_operations, student_metadata_obj)
