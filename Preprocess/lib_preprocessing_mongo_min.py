...
from Preprocess.glove_sif_io_mongo_min import *
...
def DecomposeSummary(fname, summ_ind, dir1, method, sentence_file = None, abcd_dir = None, glove_dir = None):
	...
	#1cmd = 'python sentparser.py ' + fname + '.xml ' + str(summ_ind)
	#Wasih (06-25-21) Add ABCD segmentation method
	# script_path = os.path.dirname(os.path.realpath(__file__)) 
	sentparser_script_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'sentparser_mongo_min.py') 
	if method == 'dcp':
		cmd = 'python3' + ' ' + sentparser_script_path + ' ' + fname +' ' + str(summ_ind) + ' '+ dir1
	else:
		cmd = 'python3 abcd_decomposer_mongo_min.py ' + fname + ' ' + str(summ_ind) + ' ' + dir1 + ' ' + sentence_file + ' ' + abcd_dir + ' ' + glove_dir
	os.system(cmd)
